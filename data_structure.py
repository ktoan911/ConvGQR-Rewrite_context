...
class T5RewriterIRDataset_qrecc(Dataset):
    def __init__(self, args, tokenizer, filename):
        ...
        for record in tqdm(data):
            ...
            cur_utt = tokenizer.encode(
                cur_utt_text, add_special_tokens=True, max_length=args.max_query_length
            )
            ...
            for j in range(len(ctx_utts_text) - 1, -1, -1):
                ...
                if len(flat_concat) + len(utt) > args.max_concat_length:
                    flat_concat += utt[
                        : args.max_concat_length - len(flat_concat) - 1
    ...
    def get_collate_fn(args):
        def collate_fn(batch: list):
            ...
            not_need_to_tensor_keys = set(
                ["bt_sample_ids", "bt_cur_utt_text", "bt_oracle_utt_text"]
            )
            ...
            for key in collated_dict:
                if key not in not_need_to_tensor_keys:
                    collated_dict[key] = torch.tensor(
                        collated_dict[key], dtype=torch.long
                    )
