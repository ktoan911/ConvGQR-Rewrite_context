...
from data_structure import T5RewriterIRDataset_qrecc
from utils import check_dir_exist_or_build, get_optimizer, set_seed
...
def save_model(args, model, query_tokenizer, save_model_order, epoch, step, loss):
    output_dir = oj(
        ...
        "{}-{}-best-model".format("KD-ANCE-prefix", args.decode_type),
    )
...
def train(args, log_writer):
    ...
    if args.n_gpu > 1:
        query_encoder = torch.nn.DataParallel(
            query_encoder, device_ids=list(range(args.n_gpu))
        )

    args.batch_size = args.per_gpu_train_batch_size * max(1, args.n_gpu)

    # data prepare
    train_dataset = T5RewriterIRDataset_qrecc(
        args, query_tokenizer, args.train_file_path
    )
    ...
    total_training_steps = args.num_train_epochs * (
        len(train_dataset) // args.batch_size
        + int(bool(len(train_dataset) % args.batch_size))
    )
    ...
    epoch_iterator = trange(
        args.num_train_epochs, desc="Epoch", disable=args.disable_tqdm
    )
    ...
    for epoch in epoch_iterator:
        ...
        for batch in tqdm(train_loader, desc="Step", disable=args.disable_tqdm):
            ...
            if args.print_steps > 0 and global_step % args.print_steps == 0:
                logger.info(
                    "Epoch = {}, Global Step = {}, decode loss = {}, total loss = {}".format(
                        epoch + 1,
                        global_step,
                        # ranking_loss.item(),
                        decode_loss.item(),
...
def get_args():
    ...
    parser.add_argument(
        "--train_file_path",
        type=str,
        default="datasets/qrecc/new_preprocessed/train_with_doc.json",
    )
    parser.add_argument("--log_dir_path", type=str, default="output/train_qrecc/Log")
    parser.add_argument(
        "--model_output_path", type=str, default="output/train_qrecc/Checkpoint"
    )
    ...
    parser.add_argument(
        "--num_train_epochs", type=int, default=15, help="num_train_epochs"
    )
    parser.add_argument(
        "--max_query_length", type=int, default=32, help="Max single query length"
    )
    parser.add_argument(
        "--max_doc_length", type=int, default=384, help="Max doc length"
    )
    parser.add_argument(
        "--max_response_length", type=int, default=64, help="Max response length"
    )
    parser.add_argument(
        "--max_concat_length",
        type=int,
        default=512,
        help="Max concatenation length of the session",
    )
